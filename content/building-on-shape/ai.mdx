...
We've built a dedicated MCP server for Shape, wrapping key data like Gasback distributions, NFT analytics, and Stack medals. It's open-source, forkable, and ready for your agents. Built with [xmcp](https://xmcp.dev/), it's lightweight and deploys in minutes.
...
1. Fork and clone: `git clone https://github.com/your-fork/shape-mcp-server`
...
- Streams responses for real-time chats.
- Built-in tool support: Call MCP tools like `getShapeNft` mid-conversation.
- Deploys to Vercel in one click, with Next.js for UIs.

### Rough First Steps

1. **Setup a Next.js App**: `npx create-next-app my-shape-agent`
2. **Install SDK**: `npm i ai @vercel/ai-sdk viem wagmi` (for on-chain integration) — [more info](https://ai-sdk.dev/docs/introduction).
3. **Connect MCP**: In an API route (e.g., /api/agent.ts):

   ```ts
   import { createChatAgent } from 'ai';
   import { rpcClient } from './clients'; // Your viem client

   export async function POST(req) {
     const { messages } = await req.json();
     const agent = createChatAgent({
       model: 'grok', // Or openai, claude
       tools: [
         {
           name: 'getChainStatus',
           description: 'Check Shape network status',
           execute: async () => {
             const response = await fetch('http://your-mcp-url/mcp', {
               method: 'POST',
               headers: { 'Content-Type': 'application/json' },
               body: JSON.stringify({ toolName: 'getChainStatus' }),
             });
...
## AI Agents for NFTs Hackathon

Coming soon! Build agents that supercharge Shape's NFT scene—curators, optimizers, or wild ideas. Prizes in $SHAPE. This guide is your starter kit—fork the MCP server, spin up a Vercel AI app, and experiment. Watch @shape_network on X for details.
