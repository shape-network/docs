...
3. (Optional) If you do decide to deploy the contracts and register, you'll need some Shape Sepolia ETH which you can get from the [Alchemy faucet](https://www.alchemy.com/faucets/shape-sepolia) or the [Superchain faucet](https://console.optimism.io/faucet). You will then need to create a `.env` file according to the `.env.example` file in the repo.
...
Once that's done, you can actually visit the [Gasback dashboard](https://gasback.shape.network) and see your new Gasback NFT!
